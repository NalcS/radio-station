...
unsigned char is_new_text() {
    ...
    // text file starts with 0xFE if no new text
    if (file_data[0] == 0xFE)
...
void transfer_new_text(int text_index) {
    volatile unsigned char* src = (unsigned char*)FILE_UPLOAD_BASE_ADDRESS_NON_CACHE;
    volatile unsigned char* dst = (unsigned char*)(FILE_UPLOAD_BASE_ADDRESS + text_index * FILE_ALLOC_SIZE);

    // Wait until the upload is complete (same method as wav)
    delay(500000);
